...
class cloudtorrents(object):
    ...
    def search(self, what, cat="all"):
        ...
        cat = (
            ""
            if cat == "all"
            else "&torrent_type={0}".format(self.supported_categories[cat])
        )
        current_page = 0
        while True:
            url = "{0}search?offset={1}&query={2}{3}".format(
                self.url, current_page * 25, what, cat
            )
